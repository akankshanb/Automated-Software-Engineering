...
class Div2(Pretty):
  ...
  def __init__(i,lst, x = "first", y = "last", yis="Num"):
    # print("yis check init: ", yis)
    i.yis    = yis
    i.x_lst, i.y_lst = i.createXYList(lst, yis, 0) # for zero right now
    i.b4     = i.y_lst
    i._lst   = i.y_lst.inits if i.yis == "Num" else i.y_lst.symList
    i.gain   = 0                             # where we will be, once done
    i.step   = int(i.y_lst.n**THE.div.min) # each split need >= 'step' items
    i.stop   = last(i.y_lst.inits) if i.yis == "Num" else last(i.y_lst.symList)         # top list value
    i.start  = first(i.y_lst.inits) if i.yis == "Num" else first(i.y_lst.symList)          # bottom list value
  ...
  def splitXList(i):
    ...
    i.xranges = []
    for j in i.ranges:
      for column in range(len(i.x_lst)):
        i.xranges.append(i.numSplit(i.x_lst[column].inits[start:start+j.n]))
      start += j.n
